...
Pointer::Pointer(std::string path, Window &window) {
	...
	rect.set_dimensions(std::make_tuple(texture.get_width(), texture.get_height()));
}
...
void Pointer::handle_mouse(const SDL_Event &event) {
	if (event.type == SDL_MOUSEMOTION) {
		if ((event.motion.x != rect.get_x_cord()) || event.motion.y != rect.get_y_cord()) {
			rect.set_position(std::make_tuple(event.motion.x, event.motion.y));
		}
	}
}
