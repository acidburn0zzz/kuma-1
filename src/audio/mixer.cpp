...
Mixer::Mixer() {
	...
	mode = AudioMode::Stereo, frequency = 44100;
...
void Mixer::get_spec() {
	// Mix_QuerySpec(&frequency, static_cast<uint16_t>(format),
...
void Mixer::set_mix_channels(int channels) { Mix_AllocateChannels(channels); }

void Mixer::pause_music() { Mix_PauseMusic(); }

void Mixer::resume_music() { Mix_ResumeMusic(); }

bool Mixer::music_paused() { return Mix_PausedMusic(); }
