...
void Entity::move(int constraint_x, int constraint_y, Timer &timer) {
	...
	if (timer.get_ticks() > (last_time + rate)) {
		sprite.rect.set_x_cord(sprite.rect.get_x_cord() + (sprite.rect.get_x_velocity()));
		sprite.rect.set_y_cord(sprite.rect.get_y_cord() + (sprite.rect.get_y_velocity()));
		last_time = timer.get_ticks();
	}
	if ((sprite.rect.get_x_cord() < 0) ||
	    (sprite.rect.get_x_cord() + sprite.rect.get_width() > constraint_x)) {
		sprite.rect.set_x_cord(sprite.rect.get_x_cord() - sprite.rect.get_x_velocity());
	}

	if ((sprite.rect.get_y_cord() < 0) ||
	    (sprite.rect.get_y_cord() + sprite.rect.get_height() > constraint_y)) {
		sprite.rect.set_y_cord(sprite.rect.get_y_cord() - sprite.rect.get_y_velocity());
	}
}
...
void Entity::yaml_attributes(std::string path) {
	...
	if (from["attributes"]) {
		std::for_each(from["attributes"].begin(), from["attributes"].end(),
		              [this](YAML::Node attributes) {
			              this->attributes_map[attributes["name"].as<std::string>()] =
			                  attributes["value"].as<std::string>();
			      });
	}
}

void Entity::debug_log(DebugLog &log) { log << "Entity: " << uid << " created\n"; }
